...
class CatsDtoUiModelMapper {
    fun map(listCatsdto:List<CatsDto>, listImageDto: List<ImageDto>) : List<CatsDetailUIModel>{
        return  mutableListOf<CatsDetailUIModel>().apply {
            listCatsdto.zip(listImageDto).forEach{
                add(CatsDetailUIModel(it.first.id,it.first.name,it.first.temperament,it.first.countryCode,it.first.description,it.first.wikipediaUrl,it.second.url))
            }
        }
    }
}